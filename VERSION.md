...
- **Sidebar « toujours la même partout » (demande Cissou)** : la barre du cockpit était un clone qui dérivait de celle de l'accueil (« ça ressemble mais c'est pas pareil »). Corrigé : CSS `.ck-*` ré-aligné **verbatim** sur la **source canonique** `claude-brain/projects/open-radar-fr/backend/open_radar/static/style.css` (largeur 232 px, marque conic-gradient identique, boutons Feedback/Déconnexion = `.nav-fb`/`.nav-logout`, foot à pulsation). Toute future retouche : éditer l'accueil PUIS re-synchroniser (comment en tête du bloc dans `globals.css`).
