...
export const OSIRIS_VERSION_HISTORY: { version: string; date: string; resume: string }[] = [
  ...
  },
  {
    ...
    date: '2026-07-16',
    resume:
      "⬤ Mesure : interrupteur **FOND DES CERCLES ON/OFF** (retour Cissou avant déploiement). OFF = **contour seul** " +
      "(bascule la visibilité du calque `draw-fill`), pratique pour empiler des cercles sans masquer la carte dessous. " +
      "Build+tsc verts.",
  },
  {
    version: 'V4.111-dev',
  ...
  },
  {
    ...
    date: '2026-07-13',
    resume:
      "🔎 Moteur de recherche natif — chantier ② Phase 2 (enrichissements, portage `open_radar`, parité stricte). Connecteurs " +
      "`src/lib/search/connectors2.ts` : **BODACC** (annonces civiles/commerciales par SIREN, alerte procédure collective) · " +
      "**DVF** (`api.cquest.org`, synthèse territoriale médiane €/m² + mutations, **posture non-nominative** : aucune donnée " +
      "acheteur/vendeur) · **RNA** (associations via Recherche-Entreprises `est_association=true`, identifiant W). Câblés dans " +
      "l'orchestrateur (routes rna/bodacc/foncier) → un SIREN rend entreprise + BODACC, une asso rend RNA, un « immobilier <CP> » " +
      "rend la synthèse foncière. Nouvelle route **`/company/{siren}`** (fiche SIREN = recherche standard). Build vert. Reste : " +
      "Phase 3 (person/investigate) + Phase 4 (exports/feedback).",
  },
  {
    version: 'V4.090-dev',
    date: '2026-07-13',
  ...
  },
  {
    ...
    date: '2026-07-13',
    resume:
      "🔓 Bypass auth DEV (demande Cissou) : tant que la session comptes/auth n'est pas faite (jalon Émancipation Lot C), on " +
      "GARDE la page de connexion (voir l'orga) mais « Se connecter » entre SANS pseudo/mdp (auth en standby), et un 401 ramène " +
      "sur le LoginGate V4 (jamais plus la page mot de passe V3). Gate `NEXT_PUBLIC_AUTH_BYPASS` (ON par défaut, `=0` pour " +
      "réactiver la vraie auth). Badge « mode dev · accès direct ».",
  },
  {
    version: 'V4.084-dev',
    date: '2026-07-13',
  ...
  },
  {
    ...
    date: '2026-07-12',
    resume:
      "🚨 Alertes = CONTOUR du département (demande Cissou) : chaque avis de disparition géolocalisé matérialise en plus " +
      "le tracé RÉEL de son département en rouge CLIGNOTANT (opacité pulsée ~1,4 s). 100 % hors-ligne : GeoJSON départements " +
      "FR embarqué (~248 Ko, chunk séparé chargé à la demande) + point-dans-polygone depuis la lat/lon de l'avis, zéro API " +
      "(gouv souvent bloqué VPS). V1 = niveau département + le pin gardé pour la localité exacte. Un département dessiné 1 fois même à 3 avis.",
  },
  {
    version: 'V4.077-dev',
    date: '2026-07-12',
  ...
  },
  {
    ...
    date: '2026-07-12',
    resume:
      "🐞 Fusion capsule debug (invention #15 devenue composant canonique du brain `capsules/debug-capsule/`, v1.0) : " +
      "UN seul bouton 🐞 en bas-gauche remplace le panneau V4.073. La capsule capture les erreurs client " +
      "(console, exceptions JS, promesses rejetées, fetch HTTP≥400/réseau) et génère un 📋 rapport markdown copiable " +
      "prêt à coller à un agent — plus besoin de copier du JSON à la main. L'onglet « App » rend le moniteur des sources " +
      "V4.073 (lastCount/HTTP/âge/verdicts + clés .env) via renderAppDiag. ON par défaut (staging), gate NEXT_PUBLIC_DEBUG_CAPSULE=0.",
  },
  {
    version: 'V4.073-dev',
    date: '2026-07-12',
  ...
  },
  {
    ...
    date: '2026-07-12',
    resume:
      "🔦 Vues — « QUE la France » VRAIMENT : les données d'ambiance (avions/feux/séismes/cyber/géo) sont " +
      "découpées à la FORME de la France (point-dans-polygone), plus juste au rectangle du viewport → plus de " +
      "points en Allemagne/Italie en vue France. Alertes toujours mondiales. + Voyant pour les Alertes (fraîcheur " +
      "synchro) + voyants HONNÊTES (vert = connecté ET données ; 0 donnée → orange). Géo-news : requête simplifiée.",
  },
  {
    version: 'V4.071-dev',
    date: '2026-07-12',
